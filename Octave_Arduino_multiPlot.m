...
dataStream(3).name = "t";
dataStream(3).array = [0 0];
dataStream(3).plot = 0;
dataStream(3).plcolor = "green";
dataStream(3).ylim = 0;
dataStream(3).adc_plot = [0 0];
dataStream(3).filter = 1;
...
do
   ...
   if (save_data)
     ...
     save("-text", myfilename, "dataStream");
     save_data = 0;
   endif
   ...
   if (bytesavailable > min_bytesAvailable)
     ...
     % rindex (reverse index) sucht den letzten Eintrag
     % cr_lf = [char(13) char(10)]; wird oben deklariert
     posCRLF      = index(inBuffer, cr_lf,"last");
     #posCRLF      = rindex(inBuffer, cr_lf);
     % The rindex function is equivalent to index with direction set to "last"
     % Pruefen, dass mindestens eine Zeile eingelesen wurde, d.h. ein CRLF im inBuffer ist
     if (posCRLF > 0)
       ...
       % inChar hat Struktur Name:Wert,Name:Wert\13\10Name:Wert,Name:Wert\13\10 ...
   ...
   if (x_index - x_index_prev) > min_x_index_step
     ...
     for i = 1:length(dataStream);
       if (dataStream(i).plot == 1)
          j=j+1;      % Low-Level-Plotting
          % ==================
          if (x_index > fensterbreite)
            x_axis = x_index-fensterbreite:x_index;
            dataStream(i).adc_plot = dataStream(i).array(x_index-fensterbreite:x_index);
            set(subPl(j),"xlim",[x_index - fensterbreite x_index]);
            x_index_prev = x_index;
          else % am Anfang - wenn Fester noch nicht vollgelaufen ist
            x_axis = 1:x_index;
            dataStream(i).adc_plot = dataStream(i).array;
            x_index_prev = x_index;
          endif  # x_index > fensterbreite
